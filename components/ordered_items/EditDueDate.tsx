...
import { dueSchema } from "@/lib/po-schema";
...
export function EditDueDate({ isOpen, onClose, data }: EditDialogProps) {
  ...
  const form = useForm<z.infer<typeof dueSchema>>({
    ...
    defaultValues: {
      remarks_on_supply: "",
      due_date: undefined,
      status: "Already in feedback",
    },
  });
  async function onSubmit(values: z.infer<typeof dueSchema>) {
    if (data?.create_at && values.due_date < new Date(data.create_at)) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Due date cannot be earlier than the order date",
      });
      return;
    }
    try {
